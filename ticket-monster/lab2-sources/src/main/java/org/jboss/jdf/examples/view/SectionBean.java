...
public class SectionBean implements Serializable
{
   ...
   public void retrieve()
   {
      ...
      if (this.id == null)
      {
         this.section = this.example;
      }
      else
      {
         this.section = findById(getId());
      }
   }
   ...
   private Section example = new Section();
   ...
   public Section getExample()
   {
      return this.example;
   }

   public void setExample(Section example)
   {
   ...
   private Predicate[] getSearchPredicates(Root<Section> root)
   {
      ...
      int numberOfRows = this.example.getNumberOfRows();
      if (numberOfRows != 0)
      {
         predicatesList.add(builder.equal(root.get("numberOfRows"), numberOfRows));
      }
      int rowCapacity = this.example.getRowCapacity();
      if (rowCapacity != 0)
      {
   ...
   public Converter getConverter()
   {
      ...
      final SectionBean ejbProxy = this.sessionContext.getBusinessObject(SectionBean.class);
